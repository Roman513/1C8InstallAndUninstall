...
Function SearchInstallPlatformsOnComputer {
    ...
    $Keys = $Keys +  (Get-ChildItem hklm:\software\Wow6432Node\microsoft\windows\currentversion\uninstall)
    $Items = $Keys | Foreach-Object {Get-ItemProperty $_.PsPath }
    
Foreach ($Item in $Items) {
    If ($item.Publisher -match "^(1С|1C)" -and (Test-Path -Path $Item.InstallLocation))
{
New-Object PSObject -Property @{
    DisplayName = $Item.DisplayName
    Publisher = $item.Publisher
    Version = $item.DisplayVersion
    IdentifyingNumber = $Item.PSChildName
}| select  DisplayName, Publisher, Version,  IdentifyingNumber -Unique 
}
}
}
